...
@section('content')
    <div class="text-center report-title">LAPORAN PEMBAYARAN PIUTANG</div>
    ...
    <table id="my-table" class="table display" style="margin-top: 10px">
        ...
        <tr>
            ...
            <th width="12%" class="text-center text-body-small">No. Pembayaran</th>
            <th width="12%" class="text-center text-body-small">Nota Penjualan</th>
            <th width="10%" class="text-center text-body-small">Customer</th>
            <th class="text-body-small">Keterangan</th>
            <th width="9%" class="text-right text-body-small">Nominal (Rp.)</th>
        </tr>
        </thead>
        <tbody>
        @foreach($data as $v)
            <tr>
                ...
                <td class="text-center text-body-small">{{ $v->no_transaksi }}</td>
                <td class="text-center text-body-small">{{ $v->penjualan->no_nota }}</td>
                <td class="text-center text-body-small">{{ $v->penjualan->customer }}</td>
    ...
    <hr>
    <div class="row">
        <div class="col-xs-9 f-bold text-body-small" style="text-align: right;">Total Pembayaran Piutang :</div>
        <div class="col-xs-2 f-bold text-body-small" style="text-align: right;">Rp. {{ number_format($data->sum('nominal'), 0, ',', '.') }}</div>
    </div>
    <hr>
    <div class="row">
